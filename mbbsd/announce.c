/* $Id: announce.c,v 1.5.2.1 2001/02/13 10:58:53 davidyu Exp $ */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <syslog.h>
...
static void a_showmenu(menu_t *pm) {
	...
	for(n = 0; n < p_lines && pm->page + n < pm->num; n++) {
	    ...
	    */
	    ...
	    prints("\n%6d. %-47.46s%-13s[%s]", pm->page+n+1, title, editor,
		   buf);
	}
    } else
	outs("\n  《精華區》尚在吸取天地間的日月精華中... :)");
...
static int a_pastetagpost(menu_t *pm, int mode) {
	...
	if(fhdr.filemode & FILE_TAGED) {
	    ...
	    substitute_record(dirname, &fhdr, sizeof(fhdr), ent);       
	}
    }
    fclose(fp);
    return ans;
...
static void adoent(int num, void *ent_p) {
    char *mark, *title, color;
    int type;
    time_t dtime;
    char buf[PATHLEN];
    fileheader_t *ent = (fileheader_t *)ent_p;
	...
	prints("%6d %c %-7s%-13.12s\033[1;3%cm%s %s\033[0m\n", num, type,
