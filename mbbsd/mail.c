/* $Id: mail.c,v 1.6.2.1 2001/05/01 17:17:23 davidyu Exp $ */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
int mail_all() {
    ...
    *quote_file = 0;
    ...
    strcpy(mymail.title, save_title);
    mymail.savemode = 0;
    /* mymail.filemode |= FILE_TAGED;    Ptt ¤½§i§ï¦¨¤£·|mark */
...
static void mailtitle() {
    ...
    outs("[¡ö]Â÷¶}  [¡ô,¡õ]¿ï¾Ü  [¡÷,r]¾\\Åª«H¥ó  [R]¦^«H   [x]Âà¹F  "
	 "[y]¸s²Õ¦^«H  ¨D§U[h]\n\033[7m"
...
static int mail_del_tag(int ent, fileheader_t *fhdr, char *direct) {
    ...
    getdata(1, 0, "½T©w§R°£¼Ð°O«H¥ó(Y/N)? [Y]", genbuf, 3, LCECHO);
    if(genbuf[0] != 'n') {
